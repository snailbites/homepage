// MOCK JSON
var SNAIL = SNAIL || {};

SNAIL.json = {
    'mw_searchsale' : {
        'caption' : 'Refactored the search experience in the MV* javascript pattern. Worked heavily on restyling the Madewell brand experience.',
        'img' : 'images/screenshots/mw-searchsale.png',
        'url' : 'https://www.madewell.com/search2/index.jsp?N=21&Nsrt=3',
        'index' : 0
    },
    'pdp' : {
        'caption' : 'Developed the front end functionality for an overhaul of the J.Crew, J.Crew Factory and Madewell product detail pages.',
        'img' : 'images/screenshots/jcrew-pdp.png',
        'url' : 'http://www.jcrew.com/womens_category/sweaters/Pullover/PRDOVR~46725/46725.jsp',
        'index' : 1
    },
    'espn' : {
        'caption' : 'Designed and developed within ESPN\'s content framework to produce the detail page for a brand new college recruiting section on espn.com.',
        'img' : 'images/screenshots/espn-recruiting.png',
        'url' : 'http://espn.go.com/college-sports/basketball/recruiting/school/_/id/120',
        'index' : 2
    },
    'dreamteam' : {
        'caption' : 'Created an engaging donation portal for Yankee\'s All Star first basement Mark Teixeria, and his DreamTeam25 charity.',
        'img' : 'images/screenshots/dreamteam25.png',
        'url' : 'http://dreamteam25.org/',
        'index' : 3
    },
    'leadership' : {
        'caption' : 'Designed within the existing style direction of The Leadership Room branding, to redesign and restyle the blog page.',
        'img' : 'images/screenshots/leadership.png',
        'url' : 'http://theleadershiproom.com/blog/',
        'index' : 4
    },
    'suze_orman' : {
        'caption' : 'Worked closely with an external backend team to develop the front end dashboard feature of Suze Orman\'s The Approved Card portal.',
        'img' : 'images/screenshots/suze-orman.png',
        'url' : 'http://www.theapprovedcard.com/',
        'index' : 5
    },
    'tgoodman' : {
        'caption' : 'Developed a fully responsive portfolio page for illustrator Timothy Goodman. For this early work with responsive design, my agency was awarded a CommArts Webpick of the Day.',
        'img' : 'images/screenshots/tgoodman.png',
        'url' : 'http://tgoodman.com/',
        'index' : 6
    },
    'richtu' : {
        'caption' : 'Worked closely with award-winning illustrator Rich Tu to develop a distinctly different horizontal scrolling portfolio page.',
        'img' : 'images/screenshots/richtu.png',
        'index' : 7
    }
};
